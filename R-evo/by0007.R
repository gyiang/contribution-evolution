...
contr_tag<-read.csv('repo_contr_tag_merge.csv')
contributors<-read.csv('repo_contributors_merge.csv')
authors<-contributors[,1]

  contr_tag<-contr_tag[which(contr_tag$author=="Clinton Gormley"),]
  #contr_tag<-contr_tag[which(contr_tag$author=="Jonathan R H Evans"),]
 ...
 p1<-ggplot(long_dat, aes(x = variable, y = value, colour = tag, group = tag,shape=tag)) +
    ...
    labs(title = contr_tag$author)+theme(axis.title.x=element_blank(),axis.title.y=element_blank())+
  ...
  #2=========
  t_scale<-data.frame(scale(contr_tag[,2:9], center=F,scale=T))
